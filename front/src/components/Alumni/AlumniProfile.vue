<template>
  <div>
    <div class=" w-[75%] m-auto shadow-2xl mt-[10%] bg-[rgba(245, 245, 245, 0.67)] border-[1px] border-slate-200 bg-[#ffffffaf] rounded-md" >
      <div class="text-end w-[99%]">
        <fa 
        :icon="['fas', 'user-pen']" 
        class="text-[2rem] text-sky-500 m-2 hover:shadow-lg p-[5px] cursor-pointer"
        @click="isShow"
      />
    </div>
      <div class="home flex justify-center border-green-300 w-[90%]">
        <div class="image w-[24%] h-[45vh] text-center">
          ...
          />
          <img
            v-else-if="alumniData.gender == 'Male'"
            class="w-[70%] h-[60%] m-auto mb-[20px] shadow-2xl border-cyan-400 border-4"
          ...
          />
          <img
            v-else-if="alumniData.gender == 'Female'"
            class="w-[70%] h-[60%] m-auto mb-[20px] shadow-2xl border-cyan-400 border-4"
          ...
          <div class="bg-[#fcfcfc60] w-[11%] flex justify-center absolute z-1 ml-[37px] mt-[-50px] p-[3px] text-bold">
            <label for="imageFile">
  ...
  methods: {
    ...
    getData() {
      axios.get(this.url+"alumni/1").then((res) => {
        ...
        console.log(this.alumniData.profile)
      });
    },
    isShow() {
      this.$emit('edit', true);
