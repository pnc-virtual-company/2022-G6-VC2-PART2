      ...
      <div class="home flex justify-center border-green-300 w-[90%]">
        <div class="image w-[24%] h-[45vh] text-center">
          ...
          <img
            ...
            alt=""
          />
          <div class="bg-[#fcfcfc60] w-[11%] flex justify-center absolute z-1 ml-[35px] mt-[-50px] p-[3px] text-bold">
            <label for="imageFile">
          ...
          <input type="file" @change="uploadImage(onFileSelected)" hidden id="imageFile">
        </div>
  ...
  methods: {
    uploadImage(profile){
      const fd = new FormData();
        ...
        axios.post('profile/2', fd).then((response)=>{
      ...
      // axios.put(this.url+"profile/1" , newAlumni).then(() => {
